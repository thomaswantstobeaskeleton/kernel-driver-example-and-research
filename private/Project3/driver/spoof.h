...
namespace CallSpoofer
{
	...
	{
		/* Benign tag (Orwk) - avoid "File" and other cheat-like tags */
		void* addr = ExAllocatePoolWithTag(NonPagedPool, size, 'kwrO');
		if (!addr)
			return nullptr;
	...
	{
		/* RW first, then RX - avoids RWX heuristic (EAC scans) */
		void* addr = VirtualAlloc(NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		if (!addr) return nullptr;
		memcpy(addr, func, size);
		DWORD old;
		if (!VirtualProtect(addr, size, PAGE_EXECUTE_READ, &old)) {
