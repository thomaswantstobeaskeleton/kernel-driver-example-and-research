...
namespace memory
{
	...
	{
		static const auto ntoskrnl_base = *reinterpret_cast<const char**>(std::uintptr_t(globals::PsLoadedModuleList ) + 0x30);
		...
		for ( auto entry = globals::PsLoadedModuleList; entry != globals::PsLoadedModuleList->Blink; entry = entry->Flink )
		{
	...
	void* system_export( const wchar_t* module_name, const char* export_name )
	{
		...
		return globals::RtlFindExportedRoutineByName( module->DllBase, export_name );
	}
}
