...
// icons font removed - using individual icon textures now
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Animation system for button
    static std::map<ImGuiID, float> button_animations;
    auto it_anim = button_animations.find(id);
    if (it_anim == button_animations.end()) {
        button_animations.insert({ id, 0.0f });
        it_anim = button_animations.find(id);
    }

    // Smooth animation based on state
    float target_opacity = (held && hovered) ? 1.0f : hovered ? 0.8f : 0.6f;
    it_anim->second = ImLerp(it_anim->second, target_opacity, 0.15f * (1.0f - g.IO.DeltaTime));

    // Custom button styling with animations
    ImU32 bg_color = ImColor(24, 24, 24, (int)(255 * it_anim->second));
    ImU32 border_color = ImColor(192, 203, 229, (int)(255 * (it_anim->second * 0.8f)));
    
    // Render background with rounded corners
    window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_color, style.FrameRounding);
    
    // Render border
    window->DrawList->AddRect(bb.Min, bb.Max, border_color, style.FrameRounding, 0, 1.0f);
    
    // Render highlight effect on hover
    if (hovered) {
        ImU32 highlight_color = ImColor(192, 203, 229, (int)(50 * it_anim->second));
        window->DrawList->AddRectFilled(bb.Min, bb.Max, highlight_color, style.FrameRounding);
    }
    ...
    ImU32 text_color = ImColor(192, 203, 229, (int)(255 * it_anim->second));
...
bool ImGui::TabButton(const char* label, const char* icon, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    if (selected)
        RenderFrame(bb.Min, bb.Max, ImColor(12, 15, 14), true, style.FrameRounding);
    else
        RenderFrame(bb.Min, bb.Max, ImColor(12, 15, 14), true, style.FrameRounding);
    ...
    // Icon rendering removed - icons are now handled separately with image textures
    // Just render the label text (icon parameter is ignored)
    if (!selected) {
        PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(131, 122, 134)));
    }
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5, 0.8), &bb);
    if (!selected) {
        PopStyleColor();
    }
...
bool ImGui::Keybind(const char* label, int* key, bool widget_label) {
    ...
    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(129, 120, 132)));
    if (widget_label) window->DrawList->AddText(rect.Min + ImVec2(0, 5), GetColorU32(ImGuiCol_Text), label);

    window->DrawList->AddRectFilled(key_rect.Min + ImVec2(CalcItemWidth() - 21, -1), key_rect.Max - ImVec2(2, 12), ImColor(45, 45, 45), style.FrameRounding);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, 5));
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(190, 0);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(30,15)))
        {
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(129, 120, 132)));
        SameLine(-3.4f, style.ItemInnerSpacing.x);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    SetCursorScreenPos(ImVec2(bar1_pos_x + 10, bar_pos.y + 10));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? ImColor(255, 255, 255, 255) : ImColor(129, 120, 132), g.IO.DeltaTime * 6.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));

    RenderTextClipped(text_min - ImVec2(-5, 6), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
