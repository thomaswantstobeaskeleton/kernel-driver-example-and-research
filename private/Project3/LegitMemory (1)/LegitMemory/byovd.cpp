...
static void get_exe_directory(char* out, size_t out_size)
{
    if (GetModuleFileNameA(NULL, out, (DWORD)out_size) == 0) { out[0] = '\0'; return; }
    char* last = strrchr(out, '\\');
    if (last) *(last + 1) = '\0'; else out[0] = '\0';
}
    ...
    if (is_mandatory_high_process())
    {
        ...
        if (_device == INVALID_HANDLE_VALUE)
        {
            if (!file_exists())
            {
                ...
                std::cout << "[!] Place PdFwKrnl.sys (AMD vulnerable driver) in the same folder as LegitMemory.exe." << std::endl;
                return false;
            }
...
BYOVD_PROVIDER::BYOVD_PROVIDER(const std::string& driver_name, const std::string& device_name)
{
    ...
    get_exe_directory(_driver_path, FILENAME_MAX);
